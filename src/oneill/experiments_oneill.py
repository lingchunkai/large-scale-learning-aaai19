...
def main(args=None):
    ...
    if args is None:
        ...
        args.save_path = './default_results/default_oneill/'
        args.verify=False

        args.data_path='./data/oneill/all'
        args.data_path='./data/oneill/all142857'
        args.data_path='./data/oneill/all2'
...
def Train(args):
    data = GameDataset(loadpath=args.data_path, size=args.datasize, real=True)
    ...
    net = OneillPaynet(nFeats=1, verify=args.verify)
    net = net.double()
...
class OneillPaynet(nn.Module):
    def __init__(self,
                 nFeats=[1],
                 verify=False):

        super(OneillPaynet, self).__init__()
        ...
        self.fc_lambds.weight = torch.nn.Parameter(
            torch.from_numpy(-2. * np.ones([self.num_lambdas, self.nFeats])))
